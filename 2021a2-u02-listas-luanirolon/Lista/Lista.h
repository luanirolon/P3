...
void Lista<T>::invertir() {
    ...
    inicio = anterior; // El último nodo invertido será el nuevo inicio de la lista
}

template <class T>
void Lista<T>::insertarEnMedio(T dato) {
    int tamanio = getTamanio();

    // Calcula la posición media (redondeando hacia abajo)
    int posicionMedia = tamanio / 2;

    // Inserta el elemento en la posición media
    insertar(posicionMedia, dato);
}
