...
public:
    ...
    int contarPorNivel(int nivel, NodoArbol<T> *r);
 ...
 */
template <class T>
void ArbolBinario<T>::put(T data) {
    root = put(data, root);
}
...
template <class T>
int ArbolBinario<T>::contarPorNivel(int nivel, NodoArbol<T> *r)
{
    if (r == nullptr)
    {
        return 0;
    ...
    return contarPorNivel(nivel - 1, r->getLeft()) + contarPorNivel(nivel - 1, r->getRight());
}
...
void ArbolBinario<T>::espejo(NodoArbol<T> *r)
{
    ...
    // Intercambia los nodos izquierdo y derecho
    NodoArbol<T> *temp = r->getLeft();
    r->setLeft(r->getRight());
    r->setRight(temp);

    // Aplica el mismo proceso a los subárboles izquierdo y derecho
    espejo(r->getLeft());
    espejo(r->getRight());
}
